#!/bin/bash
export PASSWORD=`cat /secret-volume/password` && touch test && echo $PASSWORD >> test && /opt/keycloak-4.8.3.Final/bin/add-user-keycloak.sh --user admin --password $PASSWORD --realm master
